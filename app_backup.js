...
const populateGrid = () => {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        if (!usedImages.includes(randomIdx)) {
            ...
            const targetGridLocations = squareNums[randomIdx];
            console.log(targetGridLocations)

            document.querySelector(targetGridLocations).innerHTML = targetCategory[randomIdx];        
    ...
    const remainingGridLocations = [];
  

    // squareNums.forEach(className => {
    //     if (!targetGridLocations.includes(className)) {
    //         remainingGridLocations.push(className);
    //     }
    // })

    // console.log(remainingGridLocations);
