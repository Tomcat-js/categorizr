...
const allImgUrls = allObjs.map(function (item) {
  return item["img"].replace("url(", "").replace(")", "").replace(/['"]+/g, "");
});
...
const defineTargetPositions = () => {
  ...
  for (let i = 0; i < 3; i++) {
    const randomGridClassIndex = Math.floor(
      Math.random() * squareClasses.length
    );
...
const assignTargetImages = () => {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (!uniqueTargetImgs.includes(randomImgIdx)) {
      ...
      document.querySelector(targetPositions[i]).style.backgroundImage =
        targetCategory[randomImgIdx].img;
    } else {
      i--;
    }
  }
...
const assignRemainingImages = () => {
  remainingPositions.forEach((position, i) => {
    document.querySelector(position).style.backgroundImage =
      remainingImages[i].img;
  });
};
...
// Display loading screen while preloading images
